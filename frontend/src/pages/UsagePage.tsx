...
export default function UsagePage() {
  ...
  return (
    <div>
      <h2 className="text-2xl font-semibold text-gray-800 mb-6">{t(lang, 'usage.overview')}</h2>

      {/* Group selector */}
      <div className="flex items-center gap-2 mb-4">
        <span className="text-sm text-gray-500">{t(lang, 'usage.group_by')}</span>
        {(['key', 'model', 'day'] as const).map(g => (
          ...
          >
            {g === 'key' ? t(lang, 'usage.group_key') : g === 'model' ? t(lang, 'usage.group_model') : t(lang, 'usage.group_day')}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-5 mb-6">
        <h3 className="text-sm font-medium text-gray-700 mb-4">{t(lang, 'usage.token_distribution')}</h3>
        {!summary?.groups?.length ? (
          <p className="text-sm text-gray-400">{t(lang, 'usage.no_data')}</p>
        ) : (
          <div className="space-y-3">
            {summary.groups.slice(0, 15).map(g => (
              <div key={g.key} className="flex items-center gap-3">
                ...
                <span className="text-xs text-gray-300 shrink-0">{tf(lang, 'usage.count_suffix', { n: g.count })}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-sm font-medium text-gray-700">{t(lang, 'usage.records_title')}</h3>
        ...
        {isLoading ? (
          ...
          <div className="p-5 text-sm text-gray-400">{t(lang, 'usage.no_usage_data')}</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-100">
                <th className="text-left text-xs font-medium text-gray-400 uppercase px-5 py-3">{t(lang, 'usage.model')}</th>
                ...
                <th className="text-left text-xs font-medium text-gray-400 uppercase px-5 py-3">{t(lang, 'usage.time')}</th>
              </tr>
            </thead>
            <tbody>
